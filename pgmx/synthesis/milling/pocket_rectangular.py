"""Rectangular contour-parallel pocket path generation for PGMX synthesis."""
...
__all__ = [
    "generate_rectangular_contour_parallel_path",
...
def generate_rectangular_contour_parallel_xyz_path(
    *,
    ...
    """Generate Maestro-like XYZ points for a rectangular pocket trajectory."""
...
def _project_anchor_to_edge(
    ...
    x, y = anchor_point
    if edge == "bottom":
        return (_clamp(x, offset, length - offset), offset)
    if edge == "top":
        return (_clamp(x, offset, length - offset), width - offset)
    if edge == "left":
        return (offset, _clamp(y, offset, width - offset))
    return (length - offset, _clamp(y, offset, width - offset))
