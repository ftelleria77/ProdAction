...
class PgmxSnapshot:
    ...
    def machine_operations(self) -> tuple[PgmxWorkingStepSnapshot, ...]:
        return tuple(
            ...
            if step.runtime_type in {"Xn", "Xmsg", "Park", "Iso"}
        )
    ...
    for workplan_index, workplan in enumerate(
        ...
        for step in executables:
            ...
            step_snapshot = PgmxWorkingStepSnapshot(
                ...
                is_xiso=(
                    sp._safe_bool(sp._text(step, "./{*}IsXiso"), False)
                    if _first_child(step, "IsXiso") is not None
                    else None
                ),
                option_parameters=sp._text(step, "./{*}OptionParameters"),
            )
