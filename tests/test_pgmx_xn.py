...
decisión de diseño; `xn=None` pide la forma nativa de Maestro.
...
from pgmx.synthesis import (
    DEFAULT_XN, build_line_spec, build_synthesis_request, synthesize_request,
)
...
def _synth(tmp: Path, name: str, *, xn, **kw) -> Path:
    ...
    synthesize_request(build_synthesis_request(
        ...
        origin_x=5.0, origin_y=5.0, origin_z=25.0, xn=xn, **kw))
    return path
...
class AuthoringTest(unittest.TestCase):
    def test_por_defecto_escribe_xn(self):
        with tempfile.TemporaryDirectory() as t:
            self.assertTrue(_has_xn(_synth(Path(t), "xn_si", xn=DEFAULT_XN, lines=[_line()])))

    def test_xn_none_no_escribe_xn(self):
        # La forma NATIVA de Maestro: por defecto el archivo no trae Xn.
        with tempfile.TemporaryDirectory() as t:
            self.assertFalse(_has_xn(_synth(Path(t), "xn_no", xn=None, lines=[_line()])))


class FooterTest(unittest.TestCase):
    """El Xn se renderiza como `M5` + park X. Sin Xn, no se emiten."""

    def _footer(self, xn) -> list[str]:
        with tempfile.TemporaryDirectory() as t:
            p = _synth(Path(t), "f", xn=xn, lines=[_line()])
            return [l.strip() for l in convert(p).replace("\r\n", "\n").split("\n")]

    def test_con_xn_emite_m5_y_park(self):
        ls = self._footer(DEFAULT_XN)
        self.assertIn("M5", ls)
    ...
    def test_sin_xn_el_resto_del_footer_no_cambia(self):
        ...
        con, sin = self._footer(DEFAULT_XN), self._footer(None)
...
class GuardTest(unittest.TestCase):
    def test_sin_xn_con_taladro_se_rechaza(self):
        ...
        with tempfile.TemporaryDirectory() as t:
            p = _synth(Path(t), "g", xn=None, lines=[_line()],
