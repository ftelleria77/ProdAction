...
from pgmx.synthesis.milling import polyline as milling_polyline
from pgmx.synthesis.milling import _curve_profile as milling_curve_profile
from pgmx.synthesis.milling import channel as milling_channel
from pgmx.synthesis.milling import contour as milling_contour
...
class PgmxSynthesisPackageTests(unittest.TestCase):
    ...
    def test_vaciado_boundary_adapts_public_pocket_spec_to_v2_contract(self) -> None:
        status = pgmx_synthesis.pocket_support_status()
        ...
        geometry, strategy, depth = pgmx_synthesis.adapt_pocket_to_contract(pocket)
    ...
    def test_modular_synthesis_packages_import_and_core_uses_common_xml(self) -> None:
        ...
        self.assertIs(core_sp._id_counter, common_xml._id_counter)
        self.assertIs(core_sp._reserve_ids, common_xml._reserve_ids)
        self.assertIs(core_sp._find_plane_ref, common_xml._find_plane_ref)
        self.assertIs(core_sp._build_point_geometry, common_xml._build_point_geometry)
        self.assertIs(core_sp._build_working_step, common_xml._build_working_step)
        self.assertIs(core_sp._build_depth_expression, common_xml._build_depth_expression)
        ...
        self.assertIs(core_sp.MachineOperationSpec, common_program.MachineOperationSpec)
        self.assertIs(core_sp.WorkplanSpec, common_program.WorkplanSpec)
        self.assertIs(core_sp.XnSpec, common_program.XnSpec)
        self.assertIs(core_sp.XmsgSpec, common_program.XmsgSpec)
        self.assertIs(core_sp.build_workplan_spec, common_program.build_workplan_spec)
        self.assertIs(core_sp.build_xn_spec, common_program.build_xn_spec)
        self.assertIs(core_sp.build_xmsg_spec, common_program.build_xmsg_spec)
        self.assertIs(core_sp._normalize_xn_spec, common_program._normalize_xn_spec)
        self.assertIs(core_sp._normalize_xmsg_spec, common_program._normalize_xmsg_spec)
        self.assertIs(core_sp._normalize_workplan_spec, common_program._normalize_workplan_spec)
        self.assertIs(core_sp._normalize_workplan_specs, common_program._normalize_workplan_specs)
        self.assertIs(core_sp._normalize_machine_operations, common_program._normalize_machine_operations)
        self.assertIs(core_sp._build_xn_step, common_program._build_xn_step)
        self.assertIs(core_sp._ensure_xn_step, common_program._ensure_xn_step)
        self.assertIs(core_sp._ensure_workplans, common_program._ensure_workplans)
        self.assertIs(core_sp._append_machine_operation, common_program._append_machine_operation)
        ...
        self.assertIs(
            core_sp._append_hydrated_machining_to_workplan,
            common_program._append_hydrated_machining_to_workplan,
        )
        self.assertIs(core_sp._append_workplan_machinings, common_program._append_workplan_machinings)
        ...
        self.assertIs(core_sp._geometry_object_type, common_geometry._geometry_object_type)
        self.assertIs(core_sp._build_geometry_from_curve_spec, common_geometry._build_geometry_from_curve_spec)
        self.assertIs(core_sp._build_boundary_curve_holder, common_geometry._build_boundary_curve_holder)
        ...
        self.assertIs(core_sp._feature_depth_value, milling_common._feature_depth_value)
        self.assertIs(core_sp._build_profile_feature, milling_common._build_profile_feature)
        ...
        self.assertIs(
            core_sp._validate_tool_sinking_length_for_spec,
            milling_common._validate_tool_sinking_length_for_spec,
        )
        self.assertIs(
            core_sp._validate_vertical_x_saw_for_milling_spec,
            milling_common._validate_vertical_x_saw_for_milling_spec,
        )
        self.assertIs(
            core_sp._validate_tool_type_for_milling_spec,
            milling_common._validate_tool_type_for_milling_spec,
        )
        ...
        self.assertIs(core_sp.ChannelSpec, milling_channel.ChannelSpec)
        self.assertIs(core_sp._HydratedChannelSpec, milling_channel._HydratedChannelSpec)
        self.assertIs(core_sp.build_channel_spec, milling_channel.build_channel_spec)
        self.assertIs(core_sp._append_channel, milling_channel._append_channel)
        self.assertIs(core_sp._build_slot_side_feature, milling_channel._build_slot_side_feature)
        self.assertIs(core_sp._normalize_channel_spec, milling_channel._normalize_channel_spec)
        self.assertIs(core_sp._hydrate_channel_spec, milling_channel._hydrate_channel_spec)
        slot = milling_channel.build_channel_spec(
            start_x=0,
            start_y=0,
            end_x=120,
            end_y=0,
        ...
        with self.assertRaisesRegex(ValueError, "longitud cero"):
            milling_channel.build_channel_spec(start_x=0, start_y=0, end_x=0, end_y=0)
        ...
        self.assertTrue(callable(milling_curve_profile._append_curve_profile_milling))
        self.assertEqual(milling_curve_profile.__all__, ["_append_curve_profile_milling"])
        ...
        self.assertIs(core_sp.ContourSpec, milling_contour.ContourSpec)
        self.assertIs(core_sp._HydratedContourSpec, milling_contour._HydratedContourSpec)
        self.assertIs(core_sp.build_contour_spec, milling_contour.build_contour_spec)
        self.assertIs(core_sp._append_contour, milling_contour._append_contour)
        self.assertIs(core_sp._normalize_contour_spec, milling_contour._normalize_contour_spec)
        self.assertIs(core_sp._hydrate_contour_spec, milling_contour._hydrate_contour_spec)
        self.assertIs(core_sp._build_squaring_outline_points, milling_contour._build_squaring_outline_points)
        self.assertIs(core_sp._build_squaring_geometry_profile, milling_contour._build_squaring_geometry_profile)
        self.assertIs(core_sp._build_squaring_toolpath_profile, milling_contour._build_squaring_toolpath_profile)
        self.assertIs(
            core_sp._reparameterize_squaring_toolpath_profile,
            milling_contour._reparameterize_squaring_toolpath_profile,
        )
        squaring = milling_contour.build_contour_spec(start_edge="borde-derecho", winding="horario")
        self.assertIsInstance(squaring, milling_contour.ContourSpec)
        ...
        self.assertIs(
            ...
            milling_pocket._build_pocket_trajectory_xyz_sequences,
        )
        self.assertIs(
            core_sp._build_pocket_trajectory_curve_specs,
            milling_pocket._build_pocket_trajectory_curve_specs,
        )
        self.assertIs(
        ...
        self.assertIs(core_sp._curve_spec_from_xyz_path, milling_pocket._curve_spec_from_xyz_path)
        self.assertIs(
