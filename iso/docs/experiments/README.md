...
| **N036** | cierre TOTAL de guardas fixtureables | **29/29 byte-idéntico**: CAD+combos, invertir+combos, UPar sobre recorrido acortado/invertido, feed vigente en salidas, pasante+estrategia/leads, ZigZag diagonal/uh0, alejamiento Lineal/velocidad, multi-fresado (lado/estrategia/leads); **estrategia+cambios = PROHIBIDO por Maestro** (guarda permanente); 3 eco pendientes de regenerar (cad_long/inv_cad/inv_mp) | `test_iso_router_combos.py` (GuardClosingN036Test), [router_line_milling.md](router_line_milling.md) §10 |
