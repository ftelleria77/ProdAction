...
| **N044** | **Eje B etapa 4: pendientes del Galceado** | **11/13 byte-idéntico** (2 fail-loud subdeterminados): CN leads LÍNEA compensados (En cota/bajada/subida) + forma EN-JUEGO (ContourSpec→polilínea del perímetro, arranque a mitad de borde) + CAD estilo B GENERAL (offset r=w/2 + arco de ángulo cualquiera en convexa / **esquina VIVA en cóncava** — respuesta a la pregunta estrella; Right→G3, Left→G2, interno CW=todo vivo). Fail-loud: cad_leads (lead CAD radio w/2, 1 fixture) y cad_zigzag (ZigZag+CAD cerrado, spec+render sin derivar) — ambos CERRADOS después por N045/N046 (validación cruzada) | `_router.py` (_poly_cad_moves/_poly_body), `_reader.py` (_contour_to_polyline), `test_iso_galceado.py` |
| **N045** | **Eje B etapa 4: el lead del contorno CAD** | **9/9 byte-idéntico**: barrido RM (1/2/3) × fresa (E001/E003/E004) — el lead CAD usa las fórmulas de ESTRATEGIA (Arco (w/2)×(RM−1), RM=1 omite; Línea (w/2)×RM) ancladas a la traza OFFSETEADA y su tangente; entrada y salida comparten centro (semicírculo). CONFIRMADO sin circularidad por `Fresado_perimetral_Ar3Cota_CAD/CN` (autoría manual): Maestro almacena SIEMPRE el lead en forma CAD y el ACC decide (false=copia / true=recalcula (w/2)×RM sobre el vértice + G42) | `_router.py` (_cad_lead_*), `test_iso_galceado.py`, [galceado_perfilado.md](galceado_perfilado.md) |
| **N046** | **Eje B etapa 4: envenenados + ZigZag CAD — CIERRE** | Envenenados: **con ACC=false el ISO COPIA la curva almacenada** (lead-in con veneno emitido, lead-out sano correcto en el MISMO ISO). ZigZag CAD **3/3 byte-idéntico**: la Z de la rampa NO es fórmula (no es función de la longitud de arco) — el converter la LEE de la curva almacenada (`stored_trajectory`) y RECOMPUTA el XY (I/J al vértice nominal); vueltas alternadas + vuelta final plana; Z en arcos solo si el REDONDEO cambia. **`Galceado.pgmx` COMPLETO byte-idéntico** (vuelta única plana pa=pr=uh=0, colineal pass-through a mitad de borde, preamble-reset decidido por la PRIMERA op, alejamiento en op última). **ETAPA 4 CERRADA** | `_router.py` (_poly_cad_zigzag_*), `pgmx/adapters.py` (_stored_trajectory_primitives), `test_iso_galceado.py`, [galceado_perfilado.md](galceado_perfilado.md) |
