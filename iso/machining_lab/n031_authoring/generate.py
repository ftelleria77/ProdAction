...
CASES = [
    ("aut_reb2",   dict(line_side_offset=2.0)),
    ("aut_long",   dict(line_is_precise=True)),
    ("aut_invert", dict(line_invert_work=True)),
    ("aut_cad_l",  dict(line_side_of_feature="Left", line_cnc_correction=False)),
    ("aut_f3s12",  dict(line_feedrate=3.0, line_spindle=12000.0)),
]
    ...
    for tag, extra in CASES:
        ...
        synthesize_request(build_synthesis_request(
            ...
            line_millings=[build_line_spec(
                ...
                line_is_through=False, line_target_depth=5.0, **extra)]))
        print(f"  {path.name}")
