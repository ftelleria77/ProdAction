...
| `B-BH-005` | side drill | perforacion/ranurado | traza | `G0 X/Y/Z`, `?%ETK[7]=3`, `G1 G9 ... F...`, `G0 ...` | taladro lateral ejecutado | `S010`, `S041`, `S043`, `S052`, `S054` |
...
| `T-BH-009` | internal boring head | top slot -> top slot | segundo slot consecutivo tras reset parcial | cabezal compartido, sierra vertical y offsets de preparacion | reset parcial de sierra; retorna a Top con `?%ETK[8]=1`, `G40`, `G17`; reposiciona desde el rapid del slot anterior al rapid del slot actual sin recomponer `B-BH-006` | `S046` |
...
| `S043` | `Prod 26-01-01 Cazaux` | cota `Left` angosta de side drill `B-BH-005` | plano `Left` sin replicacion, toolpath fijo espejo `width - geometria`, ancho inferido `<=150` | `G0 X/Y`, pausa `G4F0.500` en lateral final | cota de toolpath espejo | ISO Maestro | Regla aplicada: para `Left` angosto sin replicacion, si la cota fija de toolpath mas la geometria infiere ancho `<=150`, usar la geometria directa como cota fija lateral y ordenar por esa cota; en el emisor, si la pieza tiene ancho `<=150`, se conserva la pausa `G4F0.500` tambien antes del ultimo `Left` de mismo spindle. Esto cierra `Bano/Vanitory/Faja frontal.pgmx`. Validacion Cazaux: `80` exactos, `22` `header_only`, `2` operativos. Validacion ampliada: raiz `Pieza*` estable `217/222`, `ISO/Cocina` estable `82/84`. | Cierra `B-BH-005`; quedan `B-RH-002` y `T-XH-001` con `1` caso cada uno. |
| `S044` | `Prod 26-01-01 Cazaux` | traza `LineVertical` compensada `B-RH-002` | `LineHorizontal`/`LineVertical` con compensacion lateral, `Line Down` y `Line Up` | rapid, entrada, corte, lift, `G40` y alejamiento final | eje tangencial desde `Approach/Lift` y eje normal nominal | ISO Maestro | Regla aplicada: para perfiles lineales compensados con `approach_mode=Down` y `retract_mode=Up`, Maestro conserva el eje normal nominal del perfil, toma el eje tangencial del toolpath, agrega `1 mm` antes/despues de `Approach/Lift`, combina el desplazamiento de entrada con la bajada Z y no repite el punto final nominal. El modo `Quote` queda excluido porque regredia `Pieza_064/065`. Cierra `Cocina/mod 6 - Torre horno/Divisor_Horiz.pgmx`. Validacion Cazaux: `81` exactos, `22` `header_only`, `1` operativo. Validacion ampliada: raiz `Pieza*` estable `217/222`, `ISO/Cocina` sube a `83/84`. | Cierra `B-RH-002`; queda `T-XH-001` como unico residual operativo Cazaux. |
| `S045` | `Prod 26-01-01 Cazaux` | router -> side drill `T-XH-001` | entrada `line_milling -> side_drill` sin cambio explicito de `salida.etk_17` | `?%ETK[17]=257`, `S...M3`, `?%ETK[0]` | router lineal previo y velocidad de taladro lateral | taladro lateral activo | Regla aplicada: `_emit_side_drill_prepare_after_router` recibe `previous_family`; si viene de `line_milling` y no hay `etk_17` explicito, fuerza reactivacion de velocidad antes de la mascara lateral. Esto cierra `Cocina/mod 6 - Torre horno/Lat_Der.pgmx`. Auditoria `T-XH-001`: `50` exactos, `21` `header_only`, `0` diferencias de transicion. Validacion Cazaux: `82` exactos, `22` `header_only`, `0` operativos. Validacion ampliada: raiz `Pieza*` estable `217/222`, `ISO/Cocina` queda `84/84`. | Cierra el ultimo residual operativo de Cazaux en el corpus estudiado. |
| `S046` | `Prod 25-11-05 DeMarco` | top slot -> top slot `T-BH-009` | segundo `slot_milling` consecutivo despues de `B-BH-012` | `?%ETK[8]=1`, `G40`, `G17`, doble `G0` con `Z` desde rapid anterior a rapid actual | sierra vertical preparada, offsets de slot limpiados por reset parcial | segunda ranura superior activa sin preparacion completa | Regla aplicada: el selector clasifica `slot_milling -> slot_milling` como `T-BH-009`; el emisor evita repetir `B-BH-006`, emite el puente corto y arranca la segunda traza con `G0 X/Y/Z` desde el rapid del slot anterior. Validacion DeMarco: `301` exactos, `2` `header_only`, `33` operativos; Cazaux estable `82` exactos y `22` `header_only`; `ISO/Cocina` estable `84/84`; raiz `Pieza*` estable `217/222`. | Cierra los `18` residuales `stage:slot_milling_prepare` de DeMarco. |
| `S047` | `Prod 25-11-05 DeMarco` | top drill -> router `T-XH-002` | retorno a `line_milling` `OpenPolyline` lateral dentro de cadena router ya reingresada por `T-XH-002` | `?%ETK[7]=0`, `?%ETK[8]=1`, `G40`, limpieza `?%ETK[17]=0/M5/?%ETK[0]=0` | seleccion Top heredada de la cadena `OpenPolyline` lateral previa | router `E001` preparado despues de top drill | Regla aplicada: `_prior_router_to_top_requires_face_selection` acepta `line_milling` previo `OpenPolyline` lateral tanto con `T-RH-001` como con `T-XH-002`. Esto cierra `T-XH-002` como primer frente DeMarco; los casos restantes pasan a frentes posteriores reales. | Extiende la regla Cazaux de `T-XH-002` a cadenas DeMarco con segundo retorno `top_drill -> line_milling`. |
| `S048` | `Prod 25-11-05 DeMarco` | salida de sierra vertical `B-BH-007` en tanda `top slot -> top slot -> top drill` | primer slot de una tanda consecutiva, mirando la herramienta del top drill posterior a la tanda | lift, salida lateral completa, rapid de `T-BH-009` | estado de sierra vertical y posicion final real tras lift | reset parcial y segundo slot | Regla aplicada: `_slot_milling_transition_exit_required` mira un grupo adelante cuando el siguiente trabajo tambien es `slot_milling`; si el top drill posterior usa herramienta distinta de `001`, Maestro omite salida lateral completa desde el primer slot y `T-BH-009` reposiciona desde `cut_x`; si usa `001` o no hay top drill posterior, conserva la salida completa. | Cierra `B-BH-007` como primer frente DeMarco sin romper Cazaux, `ISO/Cocina` ni `Fondo`/lado derecho. |
| `S049` | `Prod 25-11-05 DeMarco` | traza router `B-RH-002` sin leads explicitos | `LineHorizontal`/`LineVertical` con compensacion lateral, sin approach/lift efectivo y sin estrategia | rapid `1 mm` antes del punto inicial, `G41/G42`, entrada a seguridad, bajada, corte, lift, `G40`, alejamiento `1 mm` | herramienta router activa en cadena `T-RH-001` | linea compensada ejecutada | Regla aplicada: la compensacion lateral sin leads se habilita tambien para perfiles lineales, no solo `OpenPolyline/Circle`. Los `Piso_Aplicado` DeMarco pasan a exactos; `Pieza_064/065` quedan exactas porque el modo `Quote` con leads sigue excluido de esta regla. | Cierra los `6` residuales `B-RH-002` de DeMarco. |
| `S050` | `Prod 25-11-05 DeMarco` | clasificacion `precision_only` para `B-BH-002` | diferencias de salida numerica en coordenadas `G0` de top drill, con misma secuencia operativa | `X/Y/Z/I/J/K/R`, tokens no numericos, cantidad de lineas normalizadas | candidato `B-BH-002` con coordenada calculada a milimetro | ISO Maestro con centesimas selectivas | Regla aplicada en el analisis, no en el emisor: si todas las diferencias significativas son movimientos equivalentes `G0/G1/G2/G3`, los tokens no numericos coinciden, la cantidad de lineas coincide y las coordenadas difieren hasta `0.005 mm`, el estado pasa a `precision_only`. Validacion DeMarco: `324` exactos, `2` `header_only`, `5` `precision_only`, `5` operativos, `2` no soportados, `46` sin ISO. Cazaux estable `82` exactos y `22` `header_only`; `ISO/Cocina` estable `84/84`. | Separa ruido numerico de frentes de bloque/transicion; quedan `T-BH-003` `4` y `B-BH-005` `1` como operativos reales DeMarco. |
| `S051` | `Prod 25-11-05 DeMarco` + `Prod 26-01-01 Cazaux` | pausa final `Left -> Left` en `T-BH-003` | repeticion final del mismo spindle lateral izquierdo | `G4F0.500`, ancho, largo, cota fija lateral | pausa amplia por ancho `<=150` | pausa solo en excepcion corta | Regla aplicada: la pausa final se conserva solo si no hay trabajo posterior, el cambio es `Left -> Left`, mismo spindle, eje `X`, ancho `150`, largo `<=1000` y cota fija `-130 -> -70`. Esto omite la pausa que sobraba en `4` DeMarco y conserva la de Cazaux `Bano/Vanitory/Faja frontal.pgmx`. Validacion: DeMarco `328` exactos, `2` `header_only`, `5` `precision_only`, `1` operativo; Cazaux `82` exactos, `22` `header_only`; `ISO/Cocina` `84/84`; raiz `Pieza*` estable. | Cierra `T-BH-003` en DeMarco sin reabrir Cazaux; queda `B-BH-005` como unico operativo real DeMarco. |
| `S052` | `Prod 25-11-05 DeMarco` | cota fija `Left` espejada en `B-BH-005` | dos bloques laterales `Left` separados por top drill, working steps `XBO_*` | `side_fixed`, `geometry_x`, ancho de plano | toolpath espejado respecto de geometria | geometria directa para cota fija | Regla aplicada: para `Left` sin replicacion, si el working step empieza con `XBO_` y `toolpath_raw + geometry_x` coincide con el ancho real del plano, usar `geometry_x` con signo lateral como cota fija. Cierra `Cocina\Parte 2\mod 1 - Torre heladera\Faja frontal.pgmx` donde Maestro esperaba `-80/-20` antes del top drill y `-200/-140` despues. Validacion: DeMarco `329` exactos, `2` `header_only`, `5` `precision_only`, `0` operativos; Cazaux `82` exactos, `22` `header_only`; `ISO/Cocina` `84/84`; raiz `Pieza*` estable. | Cierra el ultimo `B-BH-005` operativo de DeMarco; no quedan residuales operativos en los ISO existentes. |
| `S054` | `ISO/side_g53_z_fixtures_2026-05-03` | cota fija `Left`/`Back` en PGMX sinteticos | PGMX generados por synthesis con features `{FACE}_D{d}_{position}` (ej. `LEFT_D8_LOW`, `BACK_D8_HIGH`) | `side_fixed` | `sign * raw` (toolpath Y=240 Left, X=320 Back) | `sign * geometry.point[0]` (60 Left, 80 Back) | `G0 X/Y` con cota correcta | El generador sintetico deposita `raw = plane_x - local_x`, inverso del convenio Maestro. Para estas features la cota correcta es `sign * geometry.point[0]`, no `sign * raw`. Condicion: `feature.name` coincide con `^(LEFT|RIGHT|BACK|FRONT)_D\d`. Tests: `test_synthesis_name_check_identifies_generated_vs_maestro_features`, `test_side_fixed_for_resolved_step_uses_geometry_for_synthesis_left/back`. |
| `S053` | `Prod 25-09-04 Haeublein` | corpus ampliado de ejemplos | `135` pares PGMX/ISO con top drill, side drill y router; `26` no soportados por fresado/perfilado/arco faltante | `B-BH-002`, `B-BH-005`, reglas `PGMX-ORD-*` | reglas actuales cerradas en Cazaux/DeMarco | evidencia nueva de limites | Resultado inicial: `104` exactos, `5` operativos, `26` no soportados. Los operativos son `B-BH-002` (`4`) y `B-BH-005` (`1`). `TabiqueF6.pgmx` demuestra que la excepcion `4 huecos / una herramienta / profundidades mixtas` no siempre arranca en `max X/min Y`. `Divisor_Horiz1.pgmx` demuestra que la rotacion lateral de cara repetida no debe aplicarse globalmente: `Right -> Left -> Right` conserva el primer `Right`, a diferencia de Cazaux `Back -> Front -> Back`. | Nuevo frente recomendado: acotar `PGMX-ORD-002` y `PGMX-ORD-003` antes de tocar transiciones. |
| `S055` | `Prod 25-09-04 Haeublein` (`TabiqueF6.pgmx`) | excepcion 4 huecos `B-BH-002` condicionada a milling previo | bloque `top_drill` automatico con `4` perforaciones, una sola herramienta efectiva y profundidades mixtas, sin trabajo router o slot previo | primer `G0 X/Y`, orden vecino mas cercano | excepcion arrancaba en `max X/min Y` siempre que se cumplian los `4` criterios | excepcion solo activa si el bloque `top_drill` viene despues de `profile_milling`, `line_milling` o `slot_milling` | `TabiqueF6.pgmx` no tiene milling previo, Maestro usa origen-cercano (`origin-start`) en lugar de `max X/min Y`. En Cazaux los casos confirmados de `S042` siempre tenian router previo. Regla aplicada: `_ordered_top_drill_block` recibe `preceded_by_milling`; la excepcion solo se activa cuando ese flag es `True`. Pendiente: validacion en Maestro con fixtures `Pieza_215..218` (`tools/studies/iso/bbh002_top_drill_order_fixtures_2026_06_14.py`). |
