...
def _ordered_resolved_working_steps(snapshot: PgmxSnapshot) -> tuple[PgmxResolvedWorkingStepSnapshot, ...]:
    ...
    preceded_by_milling: bool = False
    index = 0
    while index < len(steps):
        ...
        if family == "top_drill":
            ...
            while index < len(steps) and _resolved_step_family(steps[index]) == "top_drill":
                ...
                index += 1
            ordered.extend(_ordered_top_drill_block(top_block, preceded_by_milling))
        ...
        if family == "side_drill":
            ...
            ordered.append(step)
            index += 1
            if family in {"profile_milling", "line_milling", "slot_milling"}:
...
def _ordered_top_drill_block(
    ...
    if preceded_by_milling and _top_drill_auto_block_has_single_tool_mixed_depth(block):
...
def _ordered_top_drill_nearest_neighbor(
    ...
    *,
    ...
    start_at_xy: Optional[tuple[float, float]] = None,
) -> tuple[PgmxResolvedWorkingStepSnapshot, ...]:
    remaining = sorted(block, key=_top_drill_step_sort_key)
    start_index = 0
    if start_at_xy is not None:
        ref_x, ref_y = start_at_xy
        start_index = min(
            range(len(remaining)),
            key=lambda i: (
                math.hypot(
                    _top_drill_step_sort_key(remaining[i])[0] - ref_x,
                    _top_drill_step_sort_key(remaining[i])[1] - ref_y,
                ),
                _top_drill_step_sort_key(remaining[i]),
            ),
        )
    elif start_at_max_x_min_y:
        start_index = min(
            range(len(remaining)),
...
def _side_fixed_for_resolved_step(
    ...
    if _side_drill_uses_geometry_fixed_for_synthesis(resolved_step):
        assert resolved_step.geometry is not None
        assert resolved_step.geometry.point is not None
        return float(policy["coordinate_sign"]) * float(resolved_step.geometry.point[0])
    return _side_fixed_from_toolpath(operation, policy)
...
_SYNTHESIS_SIDE_DRILL_RE = re.compile(r"^(LEFT|RIGHT|BACK|FRONT)_D\d")


def _side_drill_uses_geometry_fixed_for_synthesis(
    resolved_step: PgmxResolvedWorkingStepSnapshot,
) -> bool:
    """True when the feature was generated by synthesis, not by Maestro.

    Synthesis side-drill features follow the naming convention
    {FACE}_D{diameter}_{position} (e.g. LEFT_D8_LOW, BACK_D8_HIGH).
    For these files the toolpath approach coordinate is piece-local while
    the machine expects sign * geometry.point[0], not sign * raw.
    """
    feature = resolved_step.feature
    if feature is None:
        return False
    return bool(_SYNTHESIS_SIDE_DRILL_RE.match(feature.name or ""))
